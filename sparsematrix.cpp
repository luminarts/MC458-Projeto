...
struct TreeNode {
  ...
  TreeNode(int v = 0, int rw = 0, int col = 0, Color clr = BLACK, TreeNode *l = nullptr, TreeNode *r = nullptr, TreeNode *p = nullptr) : value(v), row(rw), column(col), color(clr), left(l), right(r), parent(p) {};
  ~TreeNode() {
    delete(left);
    delete(right);
  }
...
TreeNode* sumMatrices(TreeNode *root_a, TreeNode *root_b, bool transpose_a, bool transpose_b) {
  ...
  while (i < (int)a.size() || j < (int)b.size()) {
    ...
    if (i < (int)a.size() && j <(int)b.size() && a_row == b_row && a_col == b_col) {
      ...
      i++;
      j++;
    } else if (j >= (int)b.size() || (i < (int)a.size() && (a_row < b_row || (a_row == b_row && (a_col < b_col))))) { // Caso 2: só tem a coordenada em A
      result = insert(result, a_row, a_col, a_value);
      i++;
    } else { // Caso 3: só tem na coordenada B:
